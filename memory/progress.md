...
2026-08-10: Role-grounding round — read the Claude platform docs, then asked whether our three roles actually carry the security/infra duties the workflow assumes. They didn't. `engineer` had no security section at all (the trigger was reviewer-discovered, i.e. found too late); `manager` had escalation nowhere; `reviewer` never mentioned supply chain despite a lockfile being the highest-leverage line in any diff. Added: engineer self-declares the security trigger into the sprint contract *before* requesting review (a flag the reviewer has to discover is a defect), plus retries-compose and agent-loop-ceiling architecture bullets; manager gets escalation destination, risk acceptance, rollback-vs-forward-fix; reviewer gets supply chain with per-direct-dependency justification. All six adapter descriptions rewritten — the description *is* the routing table a delegating model reads. Injection-resistance regression suite added to `eval.md`; four agent-era bullets to `rules/security.md` (isolation buys autonomy; agent policy files are privileged; the agent's environment is readable; third-party instruction artifacts are executable code).

Ran the template's own workflow on itself, and both gates caught things a self-review would not have. The `engineer` subagent **halted at 8002 words** against the 8000 tripwire and explicitly refused to raise the threshold as sensor-gaming — correct call, I trimmed prose instead. The fresh-context `reviewer` then returned **BLOCK** twice. Round 1: `operate.md` step 5 had no owner (and it is the step backing the "every prod incident → fix, test, or rule update" non-negotiable), and `manager.md` assigned manager a write duty its own adapters forbid — the identical defect class the sprint was convened to fix for the reviewer. Round 2, after fixes: my hardened write-capability sensor was **50% inoperative** — `grep -q "Write"` matches `TodoWrite`, so the arm could never fail with `Write` deleted, and that is precisely the capability the reviewer needs to flip `passes: true`. Delimiter-anchored both arms and added the Claude-side inverse (manager must *not* hold Write/Edit). Also fixed the reviewer's non-blocking find of the same class in `daily-report.md`.

Verification: audit exit 0, 21 sensors, zero WARN/FAIL, methodology 7,989/8,000. Write-capability sensor negative-tested **8/8** (both tools × reviewer-must-write and manager-must-not, plus key-deleted variants); description coverage 6/6. `shellcheck` still unavailable locally — `bash -n` clean, CI is the real check.
...
2026-08-10: Role duties must be executable given the adapters' declared capabilities — a duty written into `aidlc/agents/*.md` that the tool adapter forbids is a silent no-op, so the audit now asserts capability positively on both tools in both directions (reviewer writes, manager does not).
2026-08-10: Substring matching is banned in capability sensors — `grep -q "Write"` passing on `TodoWrite` made a sensor report green while its invariant was regressed; delimiter-anchor every match against a comma-separated list.
