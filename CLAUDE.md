...
- **Adapter shape** (`.claude/`):
  ...
  - `rules/` — path-scoped via `paths:` frontmatter; bodies are pointers to canonical `aidlc/rules/*.md` (single source of truth shared with Cursor and Codex). One exception: `rules/project.md` ships **no** `paths:` block, so it loads unconditionally — that is how the `project.yml` declaration reaches every session. An *empty* `paths:` block is a dead rule and the audit fails it; an absent block is not.
