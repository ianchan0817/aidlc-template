...
3. **Harness review** — when the model/tooling stack changed, repeated failures appear, or eval scores saturate: re-read `aidlc/common/session-lifecycle.md`, `aidlc/construction/eval.md`, and tool adapters (`.claude/`, `.cursor/`, `.codex/`). Check guide/sensor coverage, remove scaffolding that no longer helps, add missing guardrails, and retune per model/tool. **Manager** ensures this happens at least quarterly or after a major model upgrade.
