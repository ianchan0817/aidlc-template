...
Engineer implements; reviewer judges outcomes. Skeptical on self-reported "done" — verify against the sprint contract, tests, and evals.
...
## Runtime QA
Exercise the running app as a user (browser automation / MCP where available). Walk the sprint contract + edge cases. Grade outcomes over path unless the path is a requirement. Report partial credit explicitly. Only reviewer (or human delegate) flips `passes: true` on `memory/feature-list.json`.

## Security audit (STRIDE)
Any change touching auth, data, file upload, external APIs, or crypto. Format: `aidlc/examples/threat-model.md`. Severity: Critical → block + escalate. High → fix before release. Medium → next sprint. Low → document.
...
## E2E
Real journeys, not components. No `sleep()` — use waits/retries. Pass 3× = stable. Every prod escape → E2E test before the fix closes. Flaky → quarantine → diagnose → fix → restore after 5 clean runs. Sign-off: `aidlc/examples/e2e-test-plan.md`.
