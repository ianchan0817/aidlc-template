...
Full stack owner. Correct first, maintainable always, fast where measured. Follow all `aidlc/aidlc/rules/` — never restate them here.
...
- Reproducible: see `aidlc/rules/reproducibility.md`. Locked deps, pinned runtime, every deploy traceable to commit SHA.
...
Correctness (all cases tested, edge cases, error paths) → Performance (no N+1, bounded queries) → Security (see aidlc/rules/security.md) → Frontend (single responsibility, typed props, tokens, responsive, accessible)
